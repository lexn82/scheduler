...
class PartitionSchedulerSpec extends ActorPathFreeSpec("PartitionSchedulerSpec")
  ...
  "PartitionScheduler should" - {
    ...
    "dispatch tasks due in the future later" in {
      val tasks = TaskFactory.makeTasks(2, scheduledTime = LocalDateTime.now(ZoneOffset.UTC) + 1.second)
      scheduler ! ScheduleTasks(tasks)
    ...
    "correctly merge incoming tasks" in {
      val tasks2 = TaskFactory.makeTasks(2, scheduledTime = LocalDateTime.now(ZoneOffset.UTC) + 2.seconds)
      scheduler ! ScheduleTasks(tasks2)

      val tasks1 = TaskFactory.makeTasks(2, scheduledTime = LocalDateTime.now(ZoneOffset.UTC) + 1.second)
    ...
    "overwrite tasks with the same key" in {
      val originalTask = TaskFactory.makeTask(scheduledTime = LocalDateTime.now(ZoneOffset.UTC) + 2.seconds)
    ...
    "reply to in-flight task status queries" in {
      ...
      val taskCount = 2
      val tasks = TaskFactory.makeTasks(taskCount, scheduledTime = LocalDateTime.now(ZoneOffset.UTC) + 1.second)
      scheduler ! ScheduleTasks(tasks)
      scheduler ! ThroughputController.FetchInProgressTaskCount
