...
trait TaskScheduleDao {
   ...
   * @return
   */
  def load(partitionId: PartitionId, from: TaskKey, to: LocalDateTime, limit: Int): Future[IndexedSeq[Task]]
   ...
   */
  def loadTasksFromPartitions(partitionIds: Set[PartitionId], from: LocalDateTime, to: LocalDateTime,
   ...
   * @return
   */
  def getTotalTaskCount(partitions: Set[PartitionId], from: LocalDateTime, to: LocalDateTime,
...
class TaskScheduleDaoImpl(
  ...
  def load(partitionId: PartitionId, from: LocalDateTime, to: LocalDateTime, limit: Int): Future[IndexedSeq[Task]] = {
  ...
  private def getTaskCount(partitionId: PartitionId, from: LocalDateTime, to: LocalDateTime, limit: Int): Future[Int] = {
    val rowKeys = getRowKeysExclusive(partitionId, from, to)
