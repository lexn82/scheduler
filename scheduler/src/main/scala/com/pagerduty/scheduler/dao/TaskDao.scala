...
protected[dao] trait TaskDaoImpl {
   ...
   */
  private def hourKeyFor(time: LocalDateTime) = time.format(hourFormat)
  ...
  protected implicit val taskKeySerializer = {
    ...
    def taskKeyFromTuple(tuple: (LocalDateTime, Task.OrderingId, Task.UniquenessKey)): TaskKey = {
    ...
    new ProxySerializer[TaskKey, (LocalDateTime, Task.OrderingId, Task.UniquenessKey)](
   ...
   * @return
   */
  protected def getTimeBucketKey(time: LocalDateTime): TimeBucketKey = hourKeyFor(time)
   ...
   * @return
   */
  protected def getTimeBucketKeysExclusive(from: LocalDateTime, to: LocalDateTime): Seq[TimeBucketKey] = {
    val inclusiveTo = to - 1.millisecond
    for (hour <- from.inHours to inclusiveTo.inHours) yield {
      val bucketTimeStamp = LocalDateTime.ofEpochSecond(hour * 3600, 0, ZoneOffset.UTC)
   ...
   * @return
   */
  protected def getRowKeysExclusive(partitionId: PartitionId, from: LocalDateTime, to: LocalDateTime): Seq[(PartitionId, TimeBucketKey)] = {
