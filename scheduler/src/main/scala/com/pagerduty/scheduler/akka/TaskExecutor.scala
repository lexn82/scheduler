...
object TaskExecutor {
  ...
  private case class AttemptTask(attemptNumber: Int, attemptAt: LocalDateTime)
  ...
  case class ExecutingData(attemptNumber: Int, startedAt: LocalDateTime) extends Data
  case class AttemptData(
    startedAt: LocalDateTime, finishedAt: LocalDateTime, exception: Option[Throwable]
  ) extends Data
...
class TaskExecutor(
  ...
  private def nextAttempt(attemptNumber: Int, nextAttemptAt: Option[LocalDateTime]): State = {
    val attemptAt = nextAttemptAt.getOrElse(task.scheduledTime)
    val attemptDelay = java.time.Duration.between(LocalDateTime.now(ZoneOffset.UTC), attemptAt).toScalaDuration.max(0.seconds)
  ...
  private def updateTaskAsSuccessful(data: ExecutingData): State = {
    ...
    val attemptData = AttemptData(data.startedAt, finishedAt = LocalDateTime.now(ZoneOffset.UTC), exception = None)
  ...
  private def updateTaskAsFailed(data: ExecutingData, exception: Throwable): State = {
    ...
    val attemptData = AttemptData(data.startedAt, finishedAt = LocalDateTime.now(ZoneOffset.UTC), Some(exception))
    updateTaskStatus(taskStatus, attemptData)
  }
  ...
  private def executeTask(attemptNumber: Int, attemptAt: LocalDateTime): State = {
  ...
  private def makeTaskAttempt(data: AttemptData, taskStatus: TaskStatus) = {
    TaskAttempt(
      ...
      taskResultUpdatedAt = LocalDateTime.now(ZoneOffset.UTC),
      data.exception
    )
  }
}
