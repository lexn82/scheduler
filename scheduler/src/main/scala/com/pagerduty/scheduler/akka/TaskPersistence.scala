...
object TaskPersistence {
   ...
   * @param limit
   */
  case class LoadTasks(upperBound: LocalDateTime, limit: Int) extends RetryableRequest

  /**
   * A notification from TaskPersistence sent after successful fetch from the database.
   * @param readCheckpointTime
   */
  case class TasksLoaded(readCheckpointTime: LocalDateTime) extends SuccessResponse
  ...
  case class InFlightDaoRead(readCheckpoint: TaskKey, queryUpperBound: LocalDateTime, queryLimit: Int)
    extends Data
...
class TaskPersistence(
  ...
  private def getReadCheckpointOnRestart: TaskKey = {
    val checkpointTime = LocalDateTime.now(ZoneOffset.UTC) - lookBackOnRestart
