...
class ThroughputController(
  ...
  when(AwaitingTasksLoaded) {
    case Event(results: TaskPersistence.TasksLoaded, _) => {
      ...
      val nextFetchDelay = java.time.Duration.between(LocalDateTime.now(ZoneOffset.UTC), nextFetchTime).toScalaDuration
  ...
  def loadMoreTasks(batchSize: Int) = {
    val upperBound = LocalDateTime.now(ZoneOffset.UTC) + maxLookAhead
