...
abstract class SchedulerIntegrationSpecBase
  ...
  class TestScheduler {
    ...
    var executedTaskTimes: List[LocalDateTime] = Nil
    ...
    def tasksShouldRunNow(
      ...
      tasksShouldRunAt(expectedTasks, LocalDateTime.now(ZoneOffset.UTC), expectedOopsedTasks, false)
    }

    def noTasksRunBy(by: LocalDateTime): Unit = {
      executedTasks shouldBe Nil
      val sleepForMillis = java.time.Duration.between(LocalDateTime.now(ZoneOffset.UTC) + taskLateByMax, by).toMillis
      if (sleepForMillis > 0) {
    ...
    def tasksShouldConditionBy(
      shouldBy: LocalDateTime,
      checkTaskTimes: Boolean
    )(
      shouldCondition: => Unit
    ): Unit = {
      val timeoutAt = shouldBy + schedulerWarmUpOrSlowTestTimeout
      val timeoutIn = java.time.Duration.between(LocalDateTime.now(ZoneOffset.UTC), timeoutAt).toScalaDuration.max(0.seconds)
      try {
    ...
    def taskRunner(task: Task): Unit = {
      ...
      executedTaskTimes = LocalDateTime.now(ZoneOffset.UTC) :: executedTaskTimes
