    ...
    with BeforeAndAfter
{
  ...
  "Scheduler should" - {
    "run tasks as scheduled" in {
      val tzero = LocalDateTime.now(ZoneOffset.UTC)
    ...
    "do not schedule tasks older than scheduling-grace-window" in {
      ...
      val taskA = new Task("oid7", LocalDateTime.now(ZoneOffset.UTC) - 10.seconds, "uniq-key-7", Map("m" -> "n"))
      val taskB = new Task("oid8", LocalDateTime.now(ZoneOffset.UTC) - 30.seconds, "uniq-key-8", Map("o" -> "p"))
      scheduler.scheduleTask(taskA)
    ...
    "task scheduled before the current execution time " in {
      ...
      val taskB = new Task("oid101", LocalDateTime.now(ZoneOffset.UTC) - scheduleDelta, "uniq-key-1", Map("c" -> "d"))
      scheduler.scheduleTask(taskB)
