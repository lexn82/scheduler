...
class AdminServlet(
  ...
  get(s"$apiPath/status") {
    logResponse(Ok(GetStatusResponse(s"Scheduler Admin API is running! The time is: ${LocalDateTime.now(ZoneOffset.UTC)}")))
  }
  ...
  private def parseTime(timeStr: String): Try[LocalDateTime] = {
    Try(LocalDateTime.parse(timeStr, AdminServlet.TimeFormat))
  }
...
object AdminServlet {
  ...
  val TimeFormat = DateTimeFormatter.ofPattern(TimeFormatPattern)
}
