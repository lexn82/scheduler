  ...
  "AdminServlet" - {
    ...
    "should handle a GET to /task" - {
      "when given a valid key" - {
        val key = TaskKey(LocalDateTime.now(ZoneOffset.UTC), "orderingId", "uniquenessKey")
    ...
    "should handle a GET to /tasks" - {
      ...
      val from = LocalDateTime.parse(fromString, timeFormat)
      val toString = "2016-04-07T02:00:00.000Z"
      val to = LocalDateTime.parse(toString, timeFormat)
